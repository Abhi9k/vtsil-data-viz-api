...
from datavizapi.cassandra_operations import fetchSensorData
...
def readFromDB(from_ts, to_ts):
    ...
    data = fetchSensorData(from_ts, to_ts, sids=sids, descending=False)
    keys = data.keys()
    n_channels = len(keys)
...
start_time = datetime(2019, 4, 8, 0, 0, 0)
end_time = datetime(2019, 4, 8, 23, 0, 0)
to_ts = None
sample_rate, r, num_modes, f_lim = 1024, [4, 4], 3, [1.7, 2.5]
outfile = open('/home/vtsil/oma.csv', 'w')

while True:
    from_ts = to_ts or start_time
    to_ts = from_ts + timedelta(minutes=15)
    if to_ts > end_time:
        break
    strt = datetime.now()
